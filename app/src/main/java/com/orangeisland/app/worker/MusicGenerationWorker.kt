 ...
 *
 * Runs the whole submit → poll → download → persist flow in the background,
 * so a generation survives leaving the Music Studio page or locking the device.
 * Progress is emitted via [setProgress] so the UI can observe it.
 */
class MusicGenerationWorker(
    ...
    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        ...
        try {
            repository.generate(config, request).collect { state ->
